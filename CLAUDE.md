...
- **All client identities are confidential.** `CaseStudy::getDisplayClientNameAttribute()` always returns **"Confidential Client"** — every public surface (index cards, detail hero + sidebar, home/industries related cards, schema) uses this accessor, so no client is ever named publicly. The raw `client_name` column still holds an internal descriptor (e.g. "US Healthcare Provider") for admin reference only — the admin list reads `client_name` directly, never `display_client_name`. Don't reintroduce `display_client_name` on any public view expecting a real name, and don't surface `client_name` publicly.
- **No real client/brand/place names in prose, titles, or slugs.** Older rows that named a client, product, or location were genericised and their old slugs 301-redirected in `routes/web.php` (velir, cipla*, exide, roomstays, hotelsavings, household-survey-burhanpur-khandwa, household-survey-bhopal, …). If you add or edit a case study, keep client/product/place names out of the `project_name`, prose, `seo_*`, and `slug` — technology/platform names (Sitecore, ServiceNow, Twilio, Arc GIS) are fine as stack references. When renaming a slug, add a `Route::redirect(old, new, 301)` and rename the existing DB row's slug in place (the seeder matches on slug, so a blind reseed after a slug change creates a duplicate instead of updating).
...
- **Don't ungate the `.js`-scoped entrance animations.** Every `[data-animate]` element starts at `opacity:0` and is revealed by the IntersectionObserver in `resources/js/app.js`. That `opacity:0` rule is deliberately scoped to `.js [data-animate]` in `app.css`, and the `.js` class is added by an inline nonce'd `<script>` in `layouts/app.blade.php`'s `<head>` (before the stylesheet applies). This is what keeps the whole site visible when JS fails/disabled — **don't** move the rule back to a bare `[data-animate]` selector or the page renders blank without JS. The reduced-motion override matches the same `.js` specificity on purpose.
- **Don't hard-code the "projects delivered" number** (e.g. `60+`). It's a single source of truth: `App\Models\CaseStudy::deliveredFloor()` returns a cached, rounded-down `"N+"` from the live published count so home / about / services / industries never drift from the case-studies index. Prod (~80) renders `80+`; local (~60) renders `60+`. Use the helper, not a literal.
